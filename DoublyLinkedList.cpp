...
#ifndef DOUBLE_LINKED_LIST //include guard
#define DOUBLE_LINKED_LIST //include guard
#include <iostream>
...
{
    if (position < 1 && position > item_count) //position less than 1 
    {
        return false;
    }
    else
    {
        ...
        if(position==1)
        {
            ...
            if (head != nullptr)
            {
                head->setPrevious(nullptr); //set previous to null
            }
            delete temp;
        }
        else
        {
            ...
            for (int i = 2; i < position; i++)
            {
                ...
                if (iterator->getNext()->getNext() != nullptr) //no ptr at pos
                {
                ...
                {
                    iterator->setNext(nullptr); //set to null
                }
                delete temp;   //delete from mem
            }
        }
